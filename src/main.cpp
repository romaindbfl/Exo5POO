    ...
    while (true)
{
    ...
    while (true)
    {
        ...
        Piece *pieceDepart = jeu.getPlateau().obtenirPiece(departX, departY);
        if (pieceDepart == nullptr)
        {
            cout << "Il n'y a pas de pion sur la case de départ. Veuillez réessayer." << endl;
            continue;
        }
        // Vérifier si le pion sélectionné appartient au joueur actif
        if (pieceDepart->estPieceBlanche() != joueur1Tour)
        {
            cout << "Ce n'est pas votre pion. Veuillez sélectionner une pièce appartenant à votre équipe." << endl;
            continue;
        }
        break;
    }
    ...
    else
    {
        ...
        continue; // Redemander les coordonnées de départ et d'arrivée
    }

    // Changer de joueur après un déplacement réussi
